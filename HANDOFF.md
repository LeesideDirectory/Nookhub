...
- ~~**Mobile responsiveness**~~ **FIXED (sessions 25 + 28)** — Full audit completed (session 25): critical overflow bugs fixed (ShareWidgetModal, NewConvoModal hardcoded widths), GalleryPostModal stacks on mobile, 11 new CSS utility classes added for padding reductions, touch target sizing, font scaling, and controls wrapping. Follow-up fixes in session 28 resolved a second wave of mobile issues found in live testing: (1) **Horizontal page overflow** — all CSS grids changed from bare `1fr` to `minmax(0, 1fr)` (CSS spec: `1fr` = `minmax(auto, 1fr)`, so widget content could expand grid tracks beyond the viewport); `min-width: 0` added to all grid item selectors; `overflow-x: hidden` added to `body` as safety net. (2) **Widget title truncation** — the title div was sharing a flex row with the controls, leaving only ~50px for text after the drag handle and icon; on mobile the title now takes the full first line (`width: 100%; flex: none`) and the controls wrap to a second line beneath. (3) **Public view two-column grid** — the "Preview public view" and standalone public profile grids used hardcoded inline `repeat(2, 1fr)` with no responsive override; both now use the `nook-dash-grid` class and correctly collapse to single-column on mobile.
